...
function renderStatus(data){
  ...
  // Fill config inputs
  if($("gpu_mem_fraction")) $("gpu_mem_fraction").value = data.config?.gpu_mem_fraction ?? 0.85;
  if($("gpu_slots_per_gpu")) $("gpu_slots_per_gpu").value = data.config?.gpu_slots_per_gpu ?? 1;
  if($("gpu_cpu_slots")) $("gpu_cpu_slots").value = data.config?.cpu_slots ?? 1;
...
async function saveConfig(){
  ...
  try{
    await apiJson("/api/admin/gpu/config", {
      method: "POST",
      headers: {"content-type":"application/json"},
