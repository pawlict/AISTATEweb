"""Plus (Polkomtel) Poland GSM billing parser.

Handles XLSX billing files from Plus / Polkomtel Sp. z o.o.
"""
...
_PLUS_COLUMNS: Dict[str, List[str]] = {
    "date": [
...
class PlusParser(BillingParser):
    """Parser for Plus / Polkomtel billing XLSX files."""
    ...
    DETECT_HEADER_PATTERNS = [
        ...
        r"polkomtel",
        r"plus\s*gsm",
        r"cyfrowy\s*polsat",   # parent group
    ]
    ...
    def parse_sheet(
        ...
        col_map = self.build_column_map(header_row, _PLUS_COLUMNS)

        if "date" not in col_map:
            result.warnings.append("Nie znaleziono kolumny z datą")
            return result

        result.warnings.append(f"Plus parser — kolumny: {list(col_map.keys())}")
    ...
    def _extract_subscriber_info(self, rows: List[List[Any]]) -> SubscriberInfo:
        """Extract subscriber info from header area of Plus billing."""
        import re
