"""Play (P4) Poland GSM billing parser.

Handles XLSX billing files from Play / P4 Sp. z o.o.
P4 is the legal entity behind the Play brand.
"""
...
_PLAY_COLUMNS: Dict[str, List[str]] = {
    "date": [
...
class PlayParser(BillingParser):
    """Parser for Play / P4 billing XLSX files."""
    ...
    def parse_sheet(
        ...
        col_map = self.build_column_map(header_row, _PLAY_COLUMNS)

        if "date" not in col_map:
            result.warnings.append("Nie znaleziono kolumny z datą")
            return result

        result.warnings.append(f"Play parser — kolumny: {list(col_map.keys())}")
    ...
    def _extract_subscriber_info(self, rows: List[List[Any]]) -> SubscriberInfo:
        """Extract subscriber info from header area of Play billing."""
        import re
