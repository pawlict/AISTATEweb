"""PKO Bank Polski statement parser."""
...
class PKOParser(BankParser):
    ...
    BANK_ID = "pko"
    DETECT_PATTERNS = [
        r"pko\s*bank\s*polski",
        r"pkobp",
        r"powszechna\s*kasa\s*oszcz",
        r"ipko",
        r"www\.pkobp\.pl",
    ]
    ...
    def _find_column_mapping(self, header: List[str]) -> Dict[str, int]:
        ...
        for i, cell in enumerate(header):
            ...
            if re.search(r"data\s*operacji", cell_l):
                ...
                mapping["date"] = i
            elif re.search(r"opis\s*operacji|tytu[łl]|opis", cell_l):
                mapping["title"] = i
    ...
    def parse_text(self, text: str) -> ParseResult:
        ...
        if not transactions:
            ...
            if transactions:
                ...
                warnings.append("Nie udało się wyodrębnić transakcji z tekstu PKO BP")
        return ParseResult(bank=self.BANK_ID, info=info, transactions=transactions, warnings=warnings)
