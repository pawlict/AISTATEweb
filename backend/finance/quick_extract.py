...
def extract_bank_statement_quick(text: str) -> Dict[str, Any]:
    ...
    # Opening balance
    m = re.search(r"(?:saldo\s*(?:pocz[ąa]tkowe|otwarcia))\s*:?\s*([\d\s,.\-]+)", header, re.I)
    if m:
        result["saldo_poczatkowe"] = _parse_amount(m.group(1))

    # Closing balance
    m = re.search(r"(?:saldo\s*(?:ko[ńn]cowe|zamkni[ęe]cia))\s*:?\s*([\d\s,.\-]+)", header, re.I)
    if m:
        result["saldo_koncowe"] = _parse_amount(m.group(1))

    # Available balance — look harder, multiple patterns
    avail_patterns = [
        r"(?:saldo|[śs]rodki)\s*dost[ęe]pn[eay]\s*:?\s*([\d\s,.\-]+)",
        r"(?:dost[ęe]pne\s*[śs]rodki)\s*:?\s*([\d\s,.\-]+)",
        r"(?:kwota\s*dost[ęe]pna)\s*:?\s*([\d\s,.\-]+)",
    ]
    for pattern in avail_patterns:
        m = re.search(pattern, text, re.I)  # search full text, not just header
        if m:
            result["saldo_dostepne"] = _parse_amount(m.group(1))
            break

    # Currency detection
    m = re.search(r"(?:waluta)\s*:?\s*([A-Z]{3})", header, re.I)
    if m:
